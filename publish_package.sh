...
header "Subconscious Node SDK Release Helper"
...
echo -e "    3. Dependencies will be installed and the package built"
echo -e "    4. ${BOLD}$PKG_NAME@$LOCAL_VERSION${RESET} will be published to npm"
echo ""
...
echo ""
echo -e "    ${GREEN}git tag $NEW_TAG && git push origin $NEW_TAG && pnpm install --frozen-lockfile && pnpm run build && npm publish${RESET}"
echo ""
